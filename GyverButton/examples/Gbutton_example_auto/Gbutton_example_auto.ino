/*
   ...
   isHolded() - возвращает true при удержании дольше таймаута. Возвращает один раз до следующего нажатия!!!
   isHold() - возвращает true при удержании дольше таймаута. Продолжает срабатывать, пока кнопка удерживается!
   isStep() - очень интересная функция. Начинает возвращать true после удержания, причём делает это с периодом stepTimeout. Удобно для инкрементов, см. пример
   state() - возвращает состояние кнопки (для служебных целей, заменяет digitalRead)
   
   isSingle() - возвращает true при клике. Но возвращает с паузой, так как входит в режим отработки нескольких кликов

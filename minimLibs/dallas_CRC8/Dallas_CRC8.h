#ifndef Dallas_CRC8_h
#define Dallas_CRC8_h

// #define CRC8_USE_TABLE // Uncomment to use CRC8 table

template <typename T> uint8_t CRC8_calculate(const T &data) {
	const uint8_t *pointer = (const uint8_t*) &data;
	uint16_t i = sizeof(T); 
	uint8_t crc = 0; 
	while(--i) crc = _crc_update(crc, *pointer++);
	return crc;
}

#ifdef CRC8_USE_TABLE
static const uint8_t PROGMEM _crc_table[] = {
	
};
#endif

uint8_t _crc_update(uint8_t crc, uint8_t data) {
#if defined CRC8_USE_TABLE
#pragma message "CRC8 will use the table"
	return pgm_read_byte(&_crc_table[crc ^ data]);

#elif defined (__AVR__)
#pragma message "CRC8 will be calculated on AVR-ASM"
	uint8_t counter;
	uint8_t buffer;
	asm volatile (
	"EOR %[crc_out], %[data_in] \n\t"
	"LDI %[counter], 8          \n\t"
	"LDI %[buffer], 0x8C        \n\t"
	"_loop_start_%=:            \n\t"
	"LSR %[crc_out]             \n\t"
	"BRCC _loop_end_%=          \n\t"
	"EOR %[crc_out], %[buffer]  \n\t"
	"_loop_end_%=:              \n\t"
	"DEC %[counter]             \n\t"
	"BRNE _loop_start_%="
	: [crc_out]"=r" (crc), [counter]"=d" (counter), [buffer]"=d" (buffer)
	: [crc_in]"0" (crc), [data_in]"r" (data)
	);
	return crc;

#elif defined _CRC8_ASM
#pragma message "CRC8 will be calculated on C++"
	uint8_t i = 8;
	while (i--) {
		crc = ((crc ^ data) & 1) ? (crc >> 1) ^ 0x8C : (crc >> 1);
		data >>= 1;
	}
	return crc;
#endif
}
#endif